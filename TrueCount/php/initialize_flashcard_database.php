<?php
	...
	dealer_hits_soft ENUM('yes', 'no') NOT NULL
	)";
	...
	if (mysqli_query($conn, $sql)) {
		...
		while ($numberOfDecks <=3){
			while ($hitSoft <=1){
				...
				VALUES($numberOfDecks, $hitSoftAsWord)";
	...
	$sql = "INSERT INTO player_actions(primary_action, secondary_action)
				...
				('split', 'hit'),('split', 'double'),('split', 'stay'),('surrender', 'hit'), ('surrender', 'stay'),('surrender', 'split')";
	
	if(mysqli_query($conn, $sql)){
		echo "Rows added to table player_actions<br>";
	}else{
	...
	while($ruleset_id <= 6){
			...
			while($dealer_card<=11){
				...
				switch($hand_value){
					case $hand_value <= 7:
						...
						break;
					case 8:
						if(($dealer_card == 5 || $dealer_card == 6) && $ruleset_id <= 2){
							$action = 3;
						}else{
							$action = 1;
						}
					...
					case $hand_value >= 13 && $hand_value <= 16:
						if($ruleset_id >=3 && $hand_value == 15){
							if($dealer_card == 10){
								$action = 9;
							}else if($dealer_card == 11 && ($ruleset_id == 4 || $ruleset_id == 6)){
								$action = 9;
						...
						}else if($ruleset_id >= 5 && $hand_value == 16 && $dealer_card == 9){
							$action = 9;
						}else if($ruleset_id == 2 && $hand_value == 15 && $dealer_card == 11){
							$action = 9;
						}else if($dealer_card <= 6){
							$action = 2;
						...
						break;
					case $hand_value >= 17:
						if(($ruleset_id%2 == 0) && $dealer_card == 11 && $hand_value == 17){
							$action = 10;
						}else{
							$action = 2;
						}
		...
		while($hand_value<=20){
			...
			while($dealer_card<=11){
				...
				switch($hand_value){
					case $hand_value <= 16:
						if($ruleset_id >= 3 && $dealer_card == 4){
							if($hand_value == 13 || $hand_value == 12){
								$action = 1;
							}else if($hand_value == 14 && $ruleset_id != 4){
								$action = 1;
							}else{
								$action = 3;
							}
						}else if($dealer_card >= 4 && $dealer_card <= 6){
							$action = 3;
						}else{
							$action = 1;
						}
					...
					case 18:
						if($dealer_card >= 3 && $dealer_card <= 6){
							...
							$action = 1;
						}else if(($ruleset_id == 4 || $ruleset_id == 6) && $dealer_card == 2){
							$action = 4;
						}
						else{
							$action = 2;
						...
						break;
					case $hand_value >= 19:
						if($hand_value == 19 && $dealer_card == 6 && $ruleset_id != 3 && $ruleset_id != 5){
							$action = 4;
						}else {
							$action = 2;
						}
			...
			while($dealer_card<=11){
				...
				switch($hand_value){
					case 4:
						if($dealer_card == 2){
							$action = 6;
						}else if($dealer_card == 3 && $ruleset_id >= 3){
							$action = 6;
						...
						break;
					case 6:
						if($dealer_card == 8 && $ruleset_id >= 3){
							$aciton = 1;
						}else if($dealer_card == 2 || $dealer_card == 3 || $dealer_card == 8){
							$action = 6;
					...
					case 10:
						...
						break;
					case 12:
						if($ruleset_id >= 5 && $dealer_card == 2){
							$action = 6;
						}else if($ruleset_id >=5 && $dealer_card == 7){
							$action = 1;
						}else if($dealer_card <=6){
							$action = 5;
						}else if($dealer_card == 7){
					...
					case 14:
						...
						}else if($dealer_card == 10 && $ruleset_id <=2){
							$action = 10;
						}else if($ruleset_id == 2 && $dealer_card == 11){
							$action = 9;
						}else{
						...
						break;
					case 16:
						if(($ruleset_id == 4 || $ruleset_id == 6) && $dealer_card == 11){
							$action = 11;
						}else{
							$action = 5;
						}
						break;
					case 18:
						if($ruleset_id == 2 && $dealer_card == 11){
							$action = 8;
						}else if($dealer_card == 7 || $dealer_card >= 10){
							$action = 2;
						}else{
							$action = 5;
						}
